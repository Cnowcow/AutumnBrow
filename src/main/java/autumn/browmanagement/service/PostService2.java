...
public class PostService2 {
    ...
    public void createPost(PostDTO postDTO) throws Exception {
        ...
        Treatment2 parentTreatment = null;
        Treatment2 childTreatment = null;
        ...
        if (postDTO.getParentTreatment() == null){
            ...
            if (directParentTreatment != null && !directParentTreatment.isEmpty() && directChildTreatment != null && !directChildTreatment.isEmpty()) {
                // 새 대분류 생성
                parentTreatment = new Treatment2();
                parentTreatment.setName(directParentTreatment);
                treatmentRepository2.save(parentTreatment); // 대분류 저장

                // 새 소분류 생성
                childTreatment = new Treatment2();
                childTreatment.setName(directChildTreatment);
                childTreatment.setParent(parentTreatment); // 소분류의 부모를 대분류로 설정
                treatmentRepository2.save(childTreatment); // 소분류 저장
            }
        }
        // 시술내용은 기존 값, 세부내용은 null 일 때
        else {
            parentTreatment = treatmentRepository2.findById(postDTO.getParentTreatment())
        ...
        if (postDTO.getParentTreatment() != null && postDTO.getChildTreatment() != null) {
            Treatment2 existTreatment = treatmentRepository2.findById(postDTO.getChildTreatment())
